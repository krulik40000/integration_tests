...
public class BlogApiTest {
    ...
    @Test
    public void dataIntegrityViolationExceptionByDomainLayerResultsIn409ResponseCode() throws Exception {
        Long newUserId = 1L;
        UserRequest user = new UserRequest();
        user.setEmail("john@domain.com");
        user.setFirstName("John");
        user.setLastName("Steward");
        Mockito.when(blogService.createUser(user))
               .thenThrow(new DataIntegrityViolationException("data violation exception"));
        String content = writeJson(user);
    ...
    @Test
    public void requestingForNonExistingUserResultsIn404ResponseCode() throws Exception {
        Mockito.when(finder.getUserData(0L)).thenThrow(new EntityNotFoundException());
